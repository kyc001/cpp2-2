...
bool HeroStaticAOEBullet::judge_damage(Enemy *e) {
    ...
    enemy_real_center.first = e->real_pos.first + e->real_rect.width() * 0.5;
    enemy_real_center.second = e->real_pos.second + e->real_rect.height() * 0.5;
...
std::pair<double, double> HeroDynamicBullet::getDirectionVector() {
    ...
    temp.first = target->real_pos.first - real_pos.first;
    temp.second = target->real_pos.second - real_pos.second;
...
EnemyDynamicBullet::EnemyDynamicBullet(GameMap *map_parent, Hero *target, Enemy *user, int damage) :
    ...
    real_pos.first = user->real_pos.first;
    real_pos.second = user->real_pos.second;
    _image = _image.scaled(20,20);
    real_rect.moveTo((int)real_pos.first, (int)real_pos.second);
    real_rect.setWidth(20); real_rect.setHeight(20);
    absolute_pos.first = map_parent->getAbsoluteX() + (int)real_pos.first;
    ...
    std::pair<double, double> temp;
    temp.first = target->getRealPosition().first - real_pos.first;
    temp.second = target->getRealPosition().second - real_pos.second;
    if(temp.first == 0){
        temp.first = 0.1;
    }
    if(temp.second == 0){
...
void EnemyDynamicBullet::enable() {
    Bullet::enable();
    setRealPosition(user->real_pos.first, user->real_pos.second);
    std::pair<double, double> temp;
    temp.first = target->getRealPosition().first - real_pos.first;
    temp.second = target->getRealPosition().second - real_pos.second;
    double mod = sqrt(temp.first * temp.first + temp.second * temp.second);
