...
internal bool gl_init(void *window, RenderData *renderData)
{
    ...
    // Create Programs
    {
        ...
        uint32_t fileSize = 0;
        char *shaderHeader = platform_read_file("src/rendering/shader_header.h", &fileSize);
        ...
        ASSERT(shaderHeader, "无法为着色器头文件分配空间");
        ASSERT(vertexShader, "无法为顶点着色器分配空间");
        ASSERT(fragShader, "无法为片段着色器分配空间");
