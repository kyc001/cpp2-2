                                         ...
                                         GLsizei length, const GLchar* message, const void* user)
{
  ...
  {
    KYC_ASSERT(0, "OpenGL Error: %s", message);
  }
}
#endif // DEBUG
...
internal bool init_font(int fontSize)
{
  ...
  {
    KYC_ASSERT(0, "Failed to init Freetype");
    return false;
  }
  ...
  {
    KYC_ASSERT(0, "Failed to load TTF File!");
    return false;
  }

  if(FT_Set_Pixel_Sizes(face, 0, fontSize) != 0)
  {
    KYC_ASSERT(0, "Failed to set Font Size!");
    return false;
  }
  ...
  for(char c = 32; c < 127; c++)
  {
    ...
    if(gi == 0)
    {
      KYC_ASSERT(0, "Failed to load character for TTF!");
      return false;
    }
...
internal bool gl_init(void* window, RenderData* renderData)
{
  ...
  // Fake Window Creation for WGL function loading
  {
    HWND fake_window = CreateWindowEx(0,"KYC_window_class", "window name", WS_OVERLAPPEDWINDOW,
                                      CW_USEDEFAULT, CW_USEDEFAULT, 100, 100, 0, 0,
                                      GetModuleHandle(0), 0);
  ...
  // Actual OpenGL Initialization
  {
    ...
    KYC_ASSERT(numFormats,"Should never happen?");
  ...
  // Create Programs
  {
    ...
    uint32_t fileSize = 0;
    char* shaderHeader = platform_read_file("src/graphics/shader_header.h", &fileSize);
    ...
    KYC_ASSERT(shaderHeader, "Failed to allocate Space for Shader Header");
    KYC_ASSERT(vertexShader, "Failed to allocate Space for Vertex Shader");
    KYC_ASSERT(fragShader, "Failed to allocate Space for Frag Shader");
    ...
    // Vertex Shader
    {
      ...
      // Validate if Shaders work
      {
        ...
        if(!shaderSuccess)
        {
          ...
          KYC_ASSERT(0, "Failed to compile vertex shader: %s", shaderLog);
    ...
    // Fragment Shader
    {
      ...
      // Validate if Shaders work
      {
        ...
        if(!shaderSuccess)
        {
          ...
          KYC_ASSERT(0, "Failed to compile frag shader: %s", shaderLog);
    ...
    // Validate if program works
    {
      ...
      if(!programSuccess)
      {
        ...
        KYC_ASSERT(0, "Failed to compile shader program: %s", programInfoLog);
    ...
    // Generate Image for Texture Atlas
    {
      ...
      {
        KYC_ASSERT(0, "Failed to get data for TextureID: %d", TEXTURE_ATLAS_01);
      }
    }
