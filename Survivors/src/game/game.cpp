...
#include "../core/easing_functions.h"
#include "gameData.h"
...
#include "../graphics/render_interface.cpp"
...
internal void update_level(float dt)
{
    ...
    // Spawning System
    {
        ...
        while (gameState->spawnTimer > spawnRate)
        {
            ...
            else
            {
                // if(enemy_count >= 20)
                // {
                //   // KYC_ASSERT(0, "Reached maximum amount of Enemies");
                //   return;
                // }
            }
        }
        ...
        for (int pickupIdx = 0; pickupIdx < gameState->pickups.count; pickupIdx++)
        {
            ...
            case PICKUP_TYPE_EXP_RED:
            {
                ...
                spriteID = p->type ==
                               ...
                               ? SPRITE_CRYSTAL_BLUE
                           : p->type == PICKUP_TYPE_EXP_GREEN ? SPRITE_CRYSTAL_GREEN
                                                              : SPRITE_CRYSTAL_RED;
                break;
            }
            }
            ...
            if (point_in_circle(p->pos, playerPickupCollider))
            {
                ...
                int expCount = p->type == PICKUP_TYPE_EXP_RED ? 2500 : p->type == PICKUP_TYPE_EXP_GREEN ? 50
                                                                                                        : 1;
    ...
    // vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv		HANDLE DAMAGING AREAS START		vvvvvvvvvvvvvvvvvvvvvvvvv
    {
        for (int daIdx = 0; daIdx < gameState->damagingAreas.count; daIdx++)
        {
            ...
            case WEAPON_MAGMA_RING:
            {
                ...
                for (int enemyIdx = 0; enemyIdx < gameState->enemies.count; enemyIdx++)
                {
                    ...
                    if (circle_collision(enemyCollider, puddleCollider) && enemy->magmaPuddleHitTimer <= 0.0f)
                    {
                        inflict_damage(enemy, da->damage);
                        if (enemy->hp <= 0)
                        {
        ...
        // vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv		PLAYER SKILLS START		vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
        {
            for (int weaponIdx = 0; weaponIdx < gameState->player.weapons.count; weaponIdx++)
            {
                ...
                case WEAPON_GARLIC:
                {
                    ...
                    float radius = 75.0f;
                    radius *= w->level < 2 ? 1.0f : w->level < 3 ? 1.1f
                                                : w->level < 5   ? 1.25f
                                                                 : 1.45f;
                ...
                case WEAPON_MAGMA_RING:
                {
                    ...
                    if (gameState->enemies.count)
                    {
                        if (w->timePassed >= skillCooldown)
                        {
                            ...
                            int spawnCount = w->level < 3 ? 1 : w->level < 6 ? 2
                                                                             : 3;
